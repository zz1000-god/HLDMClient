...
extern EUseSlowDownType GetUseSlowDownType(void);
...
void PM_InitTextureTypes()
{
	...
	while ( pmove->memfgets( pMemFile, fileSize, &filePos, buffer, 511 ) != NULL && (gcTextures < CTEXTURESMAX) )
	{
		...
		// get texture type
		grgchTextureType[gcTextures] = toupper((unsigned char)buffer[i++]);
...
void PM_CheckParamters( void )
{
	...
	if ( ( pmove->onground != -1 ) && ( pmove->cmd.buttons & IN_USE) && (GetUseSlowDownType() == USE_SLOWDOWN_NEW))
	{
...
#if !defined( _TFC )
	// Slow down, I'm pulling it! (a box maybe) but only when I'm standing on ground
	if ((pmove->onground != -1) && (pmove->cmd.buttons & IN_USE) && (GetUseSlowDownType() == USE_SLOWDOWN_OLD))
	{
