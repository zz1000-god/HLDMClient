...
struct NameRect {
    int x, y, w, h, playerIndex;
};
static std::vector<NameRect> g_NameRects;
static bool g_PlayerMuted[MAX_PLAYERS] = { false };
static bool g_ShowCursor = false;
...
int CHudOldScoreboard::Draw(float fTime)
{
	...
	for (int iRow = 0; iRow < scoreboard->m_iRows; ++iRow)
	{
		...
		else
		{
            ...
            // Зберігаємо координати ніку для кліків
            int nameWidth = gHUD.GetHudStringWidthWithColorTags(szName);
            g_NameRects.push_back({xpos + nameoffset, ypos, nameWidth, ROW_GAP, sorted});
	...
	if (g_ShowCursor) {
        ...
        FillRGBA(mx - 4, my - 4, 8, 8, 255, 255, 255, 200); // простий білий курсор
    }
    return 1;
}

int GetPlayerIndexAtPos(int mx, int my) {
    for (const auto& rect : g_NameRects) {
        if (mx >= rect.x && mx <= rect.x + rect.w &&
            my >= rect.y && my <= rect.y + rect.h) {
            return rect.playerIndex;
        }
    }
    return -1;
}