...
{
	int pct;
	float flvol = 0.0f; // Initialize to prevent uninitialized variable error
	int rg[3];
	int i = 0; // Initialize i as well for safety
	...
	if (m_iGeigerRange < 1000 && m_iGeigerRange > 0)
	{
		...
		if (m_iGeigerRange > 800)
		{
			...
			flvol = 0.0f;		// Initialize flvol for this case
			// No sound should play when range > 800, so we can return early
			return 1;
		}
