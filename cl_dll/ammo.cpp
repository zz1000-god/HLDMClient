...
#if !defined( _TFC )
	if (gHUD.cl_sprite->value == 2560)
		iRes = 2560;
	else if (gHUD.cl_sprite->value == 1280)
		iRes = 1280;
	else
#endif
		if (gHUD.cl_sprite->value == 640)
			iRes = 640;
		else if (gHUD.cl_sprite->value == 320)
			iRes = 320;
...
{
	if (gHUD.m_Menu.m_fMenuDisplayed && (fAdvance == FALSE) && (iDirection == 1) && gHUD.m_Menu.OnWeaponSlotSelected(iSlot))
	{
		return;
	}
